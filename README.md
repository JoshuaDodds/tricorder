...
- `segmenter.enable_motion`, `segmenter.enable_rms`, and `segmenter.enable_vad` gate the individual trigger types. When the dashboard pauses auto capture it disables RMS/VAD triggers but keeps motion active unless `segmenter.enable_motion` is also set to `false`.
