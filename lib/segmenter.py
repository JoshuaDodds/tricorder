...
class EncodingStatus:
    def __init__(self) -> None:
        ...
        self._start_callbacks: dict[int, list[Callable[[], None]]] = {}
    ...
    def register_start_callback(self, job_id: int, callback: Callable[[], None]) -> None:
        if not callable(callback):
            return
        call_immediately = False
        with self._cond:
            if job_id in self._active:
                call_immediately = True
            elif any(entry.get("id") == job_id for entry in self._pending):
                callbacks = self._start_callbacks.setdefault(job_id, [])
                callbacks.append(callback)
            else:
                # Job already processed; treat as delivered to avoid leaks.
                return
        if call_immediately:
            try:
                callback()
    ...
    def mark_started(self, job_id: int, base_name: str) -> None:
        with self._cond:
            ...
            start_callbacks = self._start_callbacks.pop(job_id, None)
            self._cond.notify_all()
        self._notify()
        if start_callbacks:
            for callback in list(start_callbacks):
                try:
                    callback()
                except Exception:
                    pass
    ...
    def mark_finished(self, job_id: int) -> None:
        ...
        with self._cond:
            ...
            self._start_callbacks.pop(job_id, None)
...
def _schedule_recordings_refresh(
    ...
    *,
    ...
    if job_id is None:
        _emit("encode_completed")
        return

    ENCODING_STATUS.register_start_callback(job_id, lambda: _emit("encode_started"))
    ENCODING_STATUS.register_completion_callback(job_id, lambda: _emit("encode_completed"))
