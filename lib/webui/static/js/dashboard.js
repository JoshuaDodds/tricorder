...
const VALID_EVENT_TYPE_FILTERS = new Set(["all", "motion", "no-motion"]);
const METRICS_PANEL_STORAGE_KEY = "tricorder.dashboard.metricsPanel";
...
const MOBILE_PLAYER_ADVANCED_STORAGE_KEY = "tricorder.dashboard.mobilePlayerAdvanced";
...
const state = {
  ...
  expandedRowPath: "",
  mobilePlayerAdvancedExpanded: false,
};
...
state.mobilePlayerAdvancedExpanded = loadMobilePlayerAdvancedPreference();
...
const metricsPanelState = {
  expanded: true,
  auto: true,
};

const METRICS_SUMMARY_PLACEHOLDER = "Metrics loading…";

const METRICS_SUMMARY_DEFAULT_LABELS = {
...
const dom = {
  ...
  metricsBody: document.getElementById("metrics-panel-body"),
  ...
  filterMotion: document.getElementById("filter-motion"),
  ...
  mobilePlayerAdvancedToggle: null,
...
function applySummaryChildren(container, nodes) {
  if (!container) {
    return;
  }
...
function normalizeSummaryEntry(key, value) {
  if (!Object.prototype.hasOwnProperty.call(metricsSummaryState, key)) {
    return null;
  }
  ...
  if (typeof value === "object" && !Array.isArray(value)) {
    const defaultLabel = METRICS_SUMMARY_DEFAULT_LABELS[key] || "";
    const rawLabel =
  ...
  if (typeof value === "number" && Number.isFinite(value)) {
    return normalizeSummaryEntry(key, { value: String(value) });
  }
  if (typeof value === "string") {
    const trimmed = value.trim();
    if (!trimmed || trimmed === "--") {
    ...
    if (colonIndex > 0 && colonIndex < trimmed.length - 1) {
      ...
      return normalizeSummaryEntry(key, { label, value: rest });
    }
    const parts = trimmed.split(/\s+/, 2);
    if (parts.length === 2) {
      const [maybeLabel, rest] = parts;
      if (rest) {
        return normalizeSummaryEntry(key, { label: maybeLabel, value: rest });
      }
    }
    return normalizeSummaryEntry(key, { value: trimmed });
  }
  return null;
}

function renderMetricsSummary() {
  const container = dom.metricsSummary;
  if (!container) {
    return;
  }
  const doc =
    (typeof container.ownerDocument !== "undefined" && container.ownerDocument)
  ...
  for (const [key, entry] of Object.entries(metricsSummaryState)) {
    if (!entry) {
      continue;
    }
    const labelText = entry.label || METRICS_SUMMARY_DEFAULT_LABELS[key] || "";
    ...
    if (chip.dataset) {
      chip.dataset.summaryKey = key;
    }
    const labelEl = doc.createElement("span");
    labelEl.className = "metrics-summary-chip-label";
    labelEl.textContent = labelText;
    chip.append(labelEl);
    if (valueText) {
      const valueEl = doc.createElement("span");
      valueEl.className = "metrics-summary-chip-value";
      valueEl.textContent = valueText;
      chip.append(valueEl);
      ariaParts.push(`${labelText} ${valueText}`);
    } else {
  ...
  let nodesToRender = chipNodes;
  const summaryCount = chipNodes.length;

  if (summaryCount === 0) {
    const placeholder = doc.createElement("span");
    placeholder.className = "metrics-summary-placeholder";
    placeholder.textContent = METRICS_SUMMARY_PLACEHOLDER;
    nodesToRender = [placeholder];
    if (container.dataset) {
      container.dataset.empty = "true";
    }
    if (typeof container.setAttribute === "function") {
      container.setAttribute("aria-label", METRICS_SUMMARY_PLACEHOLDER);
      container.setAttribute("data-summary-text", "");
      container.setAttribute("data-summary-count", "0");
    }
    ...
    const tooltipText = ariaParts.join(" • ");
    if (typeof container.setAttribute === "function") {
      container.setAttribute("aria-label", ariaText);
      container.setAttribute("data-summary-text", tooltipText);
      container.setAttribute("data-summary-count", String(summaryCount));
    }
    if ("title" in container) {
  ...
  applySummaryChildren(container, nodesToRender);
}

function setMetricsSummaryPart(key, value) {
  if (!Object.prototype.hasOwnProperty.call(metricsSummaryState, key)) {
    return;
  }
  const next = normalizeSummaryEntry(key, value);
  const current = metricsSummaryState[key];
  if (current && next) {
    if (current.label === next.label && current.value === next.value) {
      return;
    }
  } else if (!current && !next) {
    return;
  }
  metricsSummaryState[key] = next;
  renderMetricsSummary();
}

function readStoredMetricsPanelPreference() {
  try {
  ...
  try {
    const raw = window.localStorage.getItem(METRICS_PANEL_STORAGE_KEY);
    if (raw === "1" || raw === "true") {
      return true;
    }
    if (raw === "0" || raw === "false") {
...
function persistMetricsPanelPreference(expanded) {
  try {
    if (typeof window === "undefined" || !window.localStorage) {
      return;
    }
  ...
  try {
    window.localStorage.setItem(METRICS_PANEL_STORAGE_KEY, expanded ? "1" : "0");
  } catch (error) {
    /* ignore persistence errors */
  }
}

function applyMetricsPanelExpanded(expanded, { persist = false } = {}) {
  const next = Boolean(expanded);
  metricsPanelState.expanded = next;
  if (dom.metricsPanel && dom.metricsPanel.dataset) {
    dom.metricsPanel.dataset.expanded = next ? "true" : "false";
  }
  if (dom.metricsBody) {
    dom.metricsBody.hidden = !next;
  }
  if (dom.metricsToggle) {
    if (typeof dom.metricsToggle.setAttribute === "function") {
      dom.metricsToggle.setAttribute("aria-expanded", next ? "true" : "false");
    }
    dom.metricsToggle.textContent = next ? "Hide metrics" : "Show metrics";
  }
  if (persist) {
    persistMetricsPanelPreference(next);
  }
}

function initializeMetricsPanel() {
  const stored = readStoredMetricsPanelPreference();
  metricsPanelState.auto = stored === null;
  const initialExpanded = stored === null ? !layoutIsMobile() : Boolean(stored);
  applyMetricsPanelExpanded(initialExpanded);
  if (dom.metricsToggle && typeof dom.metricsToggle.addEventListener === "function") {
    dom.metricsToggle.addEventListener("click", () => {
      const next = !metricsPanelState.expanded;
      metricsPanelState.auto = false;
      applyMetricsPanelExpanded(next, { persist: true });
    });
  }
  if (mobileLayoutQuery) {
    const handleLayoutChange = () => {
      if (!metricsPanelState.auto) {
        return;
      }
      applyMetricsPanelExpanded(!layoutIsMobile());
    };
  ...
  if (typeof window !== "undefined") {
    const hooks = window.__TRICORDER_TEST_HOOKS || (window.__TRICORDER_TEST_HOOKS = {});
    hooks.metricsPanelState = metricsPanelState;
    hooks.metricsSummaryState = metricsSummaryState;
    hooks.renderMetricsSummary = renderMetricsSummary;
  }
  renderMetricsSummary();
}
...
function renderResourceStats() {
  ...
  if (dom.cpuUsage) {
    if (cpu && Number.isFinite(cpu.percent)) {
      ...
      const cpuText = `${clamped.toFixed(clamped >= 100 ? 0 : 1)}%`;
      dom.cpuUsage.textContent = cpuText;
      cpuSummary = { label: "CPU", value: cpuText };
    } else {
  ...
  if (dom.memoryUsage) {
    if (memory && Number.isFinite(memory.percent)) {
      ...
      const memoryText = `${percent.toFixed(percent >= 100 ? 0 : 1)}%`;
      dom.memoryUsage.textContent = memoryText;
      memorySummary = { label: "Memory", value: memoryText };
    } else {
  ...
  setMetricsSummaryPart("cpu", cpuSummary);
  setMetricsSummaryPart("memory", memorySummary);
}
...
function renderRecorderUptime() {
  ...
  setMetricsSummaryPart(
    "uptime",
    valueText && valueText !== "--" ? { label: "Uptime", value: valueText } : null
  );
}
...
function requestSystemHealthRefresh({ immediate = false } = {}) {
  if (immediate) {
    if (healthEventTimer !== null) {
      window.clearTimeout(healthEventTimer);
      healthEventTimer = null;
    }
  ...
  healthRefreshPending = true;
  healthEventTimer = window.setTimeout(() => {
    healthEventTimer = null;
    healthRefreshPending = false;
    fetchSystemHealth();
...
if (typeof window !== "undefined") {
  const hooks = window.__TRICORDER_TEST_HOOKS || (window.__TRICORDER_TEST_HOOKS = {});
  hooks.mobileLayoutQuery = mobileLayoutQuery;
}
...
function loadMobilePlayerAdvancedPreference() {
  try {
    if (typeof window === "undefined" || !window.localStorage) {
      return false;
    }
  ...
  try {
    const raw = window.localStorage.getItem(MOBILE_PLAYER_ADVANCED_STORAGE_KEY);
    if (!raw) {
      return false;
    }
    return raw === "1";
...
function persistMobilePlayerAdvancedPreference(expanded) {
  try {
    if (typeof window === "undefined" || !window.localStorage) {
      return;
    }
  ...
  try {
    window.localStorage.setItem(MOBILE_PLAYER_ADVANCED_STORAGE_KEY, expanded ? "1" : "0");
  } catch (error) {
    /* ignore persistence errors */
  }
}
...
function restoreFiltersFromStorage() {
  ...
  if (
    typeof stored.motion === "string" &&
    VALID_EVENT_TYPE_FILTERS.has(stored.motion)
  ) {
    next.motion = stored.motion;
  }
  state.filters = next;
}
...
function applyCaptureStatusPush(rawStatus) {
  ...
  if (status && typeof status === "object") {
    ...
    const motionSnapshot = hasEmbeddedState
      ? { ...status.motion_state }
      : {};
    const motionKeys = [
      "motion_sequence",
    ...
    if (Object.keys(motionSnapshot).length > 0) {
      state.motionState = motionSnapshot;
    } else {
  ...
  if (payload && typeof payload === "object") {
    ...
    if (sectionKey === "archival" && !archivalState.saving) {
      ...
      if (!archivalState.dirty) {
        setArchivalStatus("Updated on disk. Reset to load changes.", "info");
      }
...
function openEventStream() {
  ...
  try {
    const source = new EventSource(EVENTS_ENDPOINT, { withCredentials: true });
...
function populateFilters() {
  ...
  if (dom.filterMotion) {
    const normalizedMotion = VALID_EVENT_TYPE_FILTERS.has(state.filters.motion)
      ? state.filters.motion
      : "all";
    if (!VALID_EVENT_TYPE_FILTERS.has(state.filters.motion)) {
      state.filters.motion = normalizedMotion;
      persistFilters();
    }
    if (dom.filterMotion.value !== normalizedMotion) {
      dom.filterMotion.value = normalizedMotion;
    }
  }
...
function toFinalizedRecordingPath(partialPath) {
...
function findFinalizedRecordForPartial(record, records) {
...
function normalizeRecordingProgressRecord(progress) {
...
function updateSelectionUI(records = null) {
  ...
  setMetricsSummaryPart(
    ...
    state.selections.size > 0
      ? { label: "Selected", value: String(state.selections.size) }
      : null
  );
...
function ensureMobilePlayerAdvancedToggle() {
  if (!dom.playerCard) {
    return null;
  }
  if (dom.mobilePlayerAdvancedToggle instanceof HTMLButtonElement) {
    return dom.mobilePlayerAdvancedToggle;
  }
  const toggle = document.createElement("button");
  toggle.type = "button";
  toggle.id = "mobile-player-advanced-toggle";
  toggle.className = "mobile-player-advanced-toggle";
  toggle.hidden = true;
  toggle.setAttribute("aria-expanded", state.mobilePlayerAdvancedExpanded ? "true" : "false");
  toggle.setAttribute("aria-controls", "waveform-section clipper-container");
  toggle.addEventListener("click", () => {
    state.mobilePlayerAdvancedExpanded = !state.mobilePlayerAdvancedExpanded;
    persistMobilePlayerAdvancedPreference(state.mobilePlayerAdvancedExpanded);
    updateMobilePlayerAdvancedState();
  });
  const insertionPoint =
    dom.waveformSection || dom.playerMeta || dom.clipperContainer || dom.playerCard.firstChild;
  if (insertionPoint) {
    dom.playerCard.insertBefore(toggle, insertionPoint);
  } else {
    dom.playerCard.append(toggle);
  }
  dom.mobilePlayerAdvancedToggle = toggle;
  return toggle;
}

function updateMobilePlayerAdvancedState() {
  if (!dom.playerCard) {
    return;
  }
  const toggle = ensureMobilePlayerAdvancedToggle();
  const playerActive = dom.playerCard.dataset.active === "true" && !dom.playerCard.hidden;
  const shouldApplyMobileLayout =
    layoutIsMobile() && playerPlacement.mode === "mobile" && playerActive;

  if (shouldApplyMobileLayout) {
    dom.playerCard.dataset.mobileLayout = "true";
    dom.playerCard.dataset.mobileAdvanced = state.mobilePlayerAdvancedExpanded ? "expanded" : "collapsed";
    if (toggle) {
      toggle.hidden = false;
      toggle.setAttribute("aria-expanded", state.mobilePlayerAdvancedExpanded ? "true" : "false");
      toggle.textContent = state.mobilePlayerAdvancedExpanded
        ? "Hide waveform & trim tools"
        : "Show waveform & trim tools";
    }
  } else {
    delete dom.playerCard.dataset.mobileLayout;
    delete dom.playerCard.dataset.mobileAdvanced;
    if (toggle) {
      toggle.hidden = true;
      toggle.setAttribute("aria-expanded", state.mobilePlayerAdvancedExpanded ? "true" : "false");
    }
...
function applyExpandedState(row, expanded) {
  if (!(row instanceof HTMLElement)) {
    return;
  }
  row.dataset.expanded = expanded ? "true" : "false";
  const toggle = row.querySelector(".record-expand-toggle");
...
function collapseAllRowsExcept(exceptionRow = null) {
  if (!dom.tableBody) {
    return;
  }
  const rows = dom.tableBody.querySelectorAll("tr[data-path]");
  rows.forEach((row) => {
    if (row !== exceptionRow) {
      applyExpandedState(row, false);
    }
  });
}

function setRowExpanded(row, expanded, options = {}) {
  if (!(row instanceof HTMLElement)) {
    return;
  }
  const { collapseSiblings = true } = options;
  applyExpandedState(row, expanded);
  const path = typeof row.dataset.path === "string" ? row.dataset.path : "";
  if (expanded) {
    if (collapseSiblings && layoutIsMobile()) {
      collapseAllRowsExcept(row);
    }
    if (path) {
      state.expandedRowPath = path;
    }
  } else if (state.expandedRowPath === path) {
    state.expandedRowPath = "";
  }
}

function syncRowExpansionState() {
  if (!dom.tableBody) {
    return;
  }
  const rows = dom.tableBody.querySelectorAll("tr[data-path]");
  ...
  if (!layoutIsMobile()) {
    rows.forEach((row) => applyExpandedState(row, true));
    state.expandedRowPath = "";
    return;
  }
  let targetPath = state.expandedRowPath;
  if (!targetPath && state.current && state.current.path) {
    targetPath = state.current.path;
  }
  let targetRow = null;
  rows.forEach((row) => {
    if (!targetRow && row.dataset.path === targetPath) {
      targetRow = row;
    }
  });
  rows.forEach((row) => {
    applyExpandedState(row, row === targetRow);
  });
  state.expandedRowPath = targetRow ? targetRow.dataset.path || "" : "";
}
...
function renderRecords() {
  ...
  if (state.expandedRowPath) {
    const hasExpandedRecord = records.some((record) => record.path === state.expandedRowPath);
  ...
  for (const record of records) {
    ...
    expandToggle.className = "record-expand-toggle";
    expandToggle.setAttribute("aria-expanded", "false");
    expandToggle.textContent = "Show details";
    expandToggle.addEventListener("click", (event) => {
      event.stopPropagation();
      const isExpanded = row.dataset.expanded === "true";
      if (isExpanded && state.current && state.current.path === record.path) {
        setNowPlaying(null);
      }
      setRowExpanded(row, !isExpanded);
    });
    nameCell.append(expandToggle);
    ...
    const shouldExpand =
      layoutIsMobile() &&
      ((state.current && state.current.path === record.path) || state.expandedRowPath === record.path);
    setRowExpanded(row, shouldExpand, { collapseSiblings: false });
  }
...
function updateStats() {
  ...
  setMetricsSummaryPart("recordings", { label: "Recordings", value: recordingsText });
  const selectedTotal = state.selections ? state.selections.size : 0;
  setMetricsSummaryPart(
    "selected",
    selectedTotal > 0 ? { label: "Selected", value: String(selectedTotal) } : null
  );
  ...
  setMetricsSummaryPart(
    "storage",
    Number.isFinite(freeHint)
      ? { label: "Free", value: formatBytes(freeHint) }
      : null
  );
  ...
  if (state.lastUpdated instanceof Date) {
    const formatted = dateFormatter.format(state.lastUpdated);
    if (dom.lastUpdated) {
      dom.lastUpdated.textContent = formatted;
    }
    setMetricsSummaryPart("lastUpdated", { label: "Updated", value: formatted });
  } else {
    if (dom.lastUpdated) {
      dom.lastUpdated.textContent = "--";
    }
    setMetricsSummaryPart("lastUpdated", null);
  }
}
...
async function fetchRecordings(options = {}) {
  ...
  if (
    typeof state.filters.motion === "string" &&
    state.filters.motion !== "all" &&
    VALID_EVENT_TYPE_FILTERS.has(state.filters.motion)
  ) {
    params.set("motion", state.filters.motion);
  }
  ...
  try {
    ...
    const payloadMotion =
      typeof payload.motion_filter === "string" &&
      VALID_EVENT_TYPE_FILTERS.has(payload.motion_filter)
        ? payload.motion_filter
        : "all";
    if (payloadMotion !== state.filters.motion) {
      state.filters = {
        ...state.filters,
    ...
    setMetricsSummaryPart("lastUpdated", { label: "Status", value: "Offline" });
...
function applyFiltersFromInputs() {
  ...
  let motion = state.filters.motion;
  if (dom.filterMotion) {
    const rawMotion = dom.filterMotion.value.trim();
    motion = VALID_EVENT_TYPE_FILTERS.has(rawMotion) ? rawMotion : "all";
  }
  const nextFilters = {
    search,
    day,
  ...
  const changed =
    ...
    nextFilters.motion !== state.filters.motion;
  ...
  if (dom.filterMotion && dom.filterMotion.value !== nextFilters.motion) {
    dom.filterMotion.value = nextFilters.motion;
  }
  if (dom.filterLimit) {
...
function initialize() {
  ...
  updatePaginationControls();
  resetWaveform();
  ensureMobilePlayerAdvancedToggle();
  updateMobilePlayerAdvancedState();
