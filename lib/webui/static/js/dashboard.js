...
function applyTabRecordingIndicatorTitle(showIndicator) {
  ...
  document.title = showIndicator ? `● Recording • ${baseTitle}` : baseTitle;
}
...
function refreshTabRecordingTitleIndicator() {
  ...
  const shouldFlash = tabRecordingTitleState.active && visibilityState === "hidden";
  if (!shouldFlash) {
...
function setTabRecordingActive(active) {
  const nextActive = Boolean(active);
  if (tabRecordingTitleState.active === nextActive) {
...
function applyRecordingIndicator(state, message, { motion = false } = {}) {
  setTabRecordingActive(state === "active");
...
async function fetchRecordings(options = {}) {
  ...
  try {
    ...
    if (!recordsChanged && partialChanged && nextPartial && nextPartial.path) {
      handledPartialUpdate = updateInProgressRecordRow(nextPartial);
    }
    if (recordsChanged || (!handledPartialUpdate && partialChanged)) {
      renderRecords();
    } else {
      updateSelectionUI();
      applyNowPlayingHighlight();
