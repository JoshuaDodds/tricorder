...
const TRIGGER_CONFIG = {
  motion: {
    ...
    stateEnabledKey: "motionEnabled",
    statePreferredKey: "motionPreferred",
    responseField: "trigger_motion_enabled",
    preferredField: "preferred_trigger_motion_enabled",
  },
  rms: {
    label: "RMS trigger",
    stateEnabledKey: "rmsEnabled",
    statePreferredKey: "rmsPreferred",
    responseField: "trigger_rms_enabled",
    preferredField: "preferred_trigger_rms_enabled",
  },
  vad: {
    label: "VAD trigger",
    stateEnabledKey: "vadEnabled",
    statePreferredKey: "vadPreferred",
    responseField: "trigger_vad_enabled",
    preferredField: "preferred_trigger_vad_enabled",
  },
};

const TRIGGER_KEYS = Object.keys(TRIGGER_CONFIG);

const triggerDisableReasons = Object.fromEntries(
  TRIGGER_KEYS.map((key) => [key, new Map()])
);
...
function getTriggerButton(key) {
  ...
  if (key === "motion") {
    return dom.triggerMotionToggle ?? null;
  }
  if (key === "rms") {
    return dom.triggerRmsToggle ?? null;
  }
  if (key === "vad") {
    return dom.triggerVadToggle ?? null;
  }
  return null;
}

function applyTriggerDisabledState(key) {
  const button = getTriggerButton(key);
  if (!button) {
    return;
  }
  const reasons = triggerDisableReasons[key];
  const disabled = reasons && reasons.size > 0;
  if (button.disabled !== disabled) {
    button.disabled = disabled;
  }
  if (disabled) {
    const reasonText = Array.from(reasons.values()).filter(Boolean).join(" ");
    if (reasonText) {
...
function setTriggerToggleDisabled(key, source, disabled, reason = "") {
  const reasons = triggerDisableReasons[key];
  if (!reasons) {
    return;
  }
  ...
  applyTriggerDisabledState(key);
}

function setTriggerTogglePending(key, pending, message = "") {
  const button = getTriggerButton(key);
  if (!button) {
    return;
  }
  if (pending) {
    ...
    setTriggerToggleDisabled(key, "pending", true, message || "Updating trigger…");
  } else {
    button.removeAttribute("aria-busy");
    setTriggerToggleDisabled(key, "pending", false);
  }
}

function setTriggerButtonState(key, active, options = {}) {
  const button = getTriggerButton(key);
  if (!button) {
    return;
  }
  ...
  if (options.preference !== undefined) {
    button.dataset.preference = options.preference ? "true" : "false";
  } else if (Object.prototype.hasOwnProperty.call(button.dataset, "preference")) {
    delete button.dataset.preference;
  }
  if (options.effective !== undefined) {
    button.dataset.effective = options.effective ? "true" : "false";
  } else if (Object.prototype.hasOwnProperty.call(button.dataset, "effective")) {
    delete button.dataset.effective;
  }
  if (options.suppressed) {
    button.dataset.suppressed = "true";
  } else {
    delete button.dataset.suppressed;
  }
}

function updateTriggerToggleStates() {
  for (const key of TRIGGER_KEYS) {
    const config = TRIGGER_CONFIG[key];
    if (!config) {
      continue;
    }
    const preferred = Boolean(autoRecordState[config.statePreferredKey]);
    const effective = Boolean(autoRecordState[config.stateEnabledKey]);
    const suppressed = preferred && !effective;
    setTriggerButtonState(key, preferred, {
...
function resolveAutoRecordAvailability(status) {
  if (!status || typeof status !== "object") {
    return { disabled: true, reason: "Recorder status unavailable." };
  }
  if (!parseBoolean(status.service_running)) {
    const stopReason =
      typeof status.last_stop_reason === "string"
        ? status.last_stop_reason.trim()
        : "";
    return { disabled: true, reason: stopReason || "Recorder service is stopped." };
  }
  return { disabled: false, reason: "" };
}

function applyTriggerAvailability(status) {
  const { disabled, reason } = resolveAutoRecordAvailability(status);
  for (const key of TRIGGER_KEYS) {
    setTriggerToggleDisabled(key, "status", disabled, reason);
  }
}

function applyAutoRecordSnapshot(status) {
  if (!status || typeof status !== "object") {
    return;
  }
  if (Object.prototype.hasOwnProperty.call(status, "auto_recording_enabled")) {
    autoRecordState.enabled = parseBoolean(status.auto_recording_enabled);
  }
  if (Object.prototype.hasOwnProperty.call(status, "auto_record_motion_override")) {
  ...
  for (const key of TRIGGER_KEYS) {
    ...
    if (Object.prototype.hasOwnProperty.call(status, config.responseField)) {
      autoRecordState[config.stateEnabledKey] = parseBoolean(
        status[config.responseField],
      );
    }
    if (Object.prototype.hasOwnProperty.call(status, config.preferredField)) {
      autoRecordState[config.statePreferredKey] = parseBoolean(
        status[config.preferredField],
      );
    } else if (
      autoRecordState[config.statePreferredKey] === undefined &&
      Object.prototype.hasOwnProperty.call(status, config.responseField)
    ) {
      autoRecordState[config.statePreferredKey] = Boolean(
        autoRecordState[config.stateEnabledKey],
      );
    }
  }
  setAutoRecordButtonState(autoRecordState.enabled);
  updateTriggerToggleStates();
}
...
function applyAutoRecordResponse(payload, { mergeIntoState = true } = {}) {
  if (!payload || typeof payload !== "object") {
    return null;
  }
  const snapshot = {};
  if (Object.prototype.hasOwnProperty.call(payload, "enabled")) {
    snapshot.auto_recording_enabled = payload.enabled;
  }
  for (const key of TRIGGER_KEYS) {
    const config = TRIGGER_CONFIG[key];
    if (!config) {
    ...
    if (Object.prototype.hasOwnProperty.call(payload, config.responseField)) {
      snapshot[config.responseField] = payload[config.responseField];
    }
    if (Object.prototype.hasOwnProperty.call(payload, config.preferredField)) {
      snapshot[config.preferredField] = payload[config.preferredField];
    }
  }
  ...
  if (mergeIntoState) {
    updateDashboardState((draft) => {
      const base =
        draft.captureStatus && typeof draft.captureStatus === "object"
          ? draft.captureStatus
          : {};
      draft.captureStatus = { ...base, ...snapshot };
    }, "capture:auto-record:response");
    updateAutoRecordButton(state.captureStatus);
  }
  return snapshot;
}

function getTriggerPendingMessage(key, nextEnabled) {
  const config = TRIGGER_CONFIG[key];
  const label = config ? config.label : "Trigger";
  return nextEnabled ? `Enabling ${label}…` : `Disabling ${label}…`;
}

async function handleTriggerToggleClick(key) {
  const config = TRIGGER_CONFIG[key];
  if (!config) {
    return;
  }
  const button = getTriggerButton(key);
  if (!button || button.disabled || autoRecordState.pending) {
    return;
  }
  const currentPreferred = Boolean(autoRecordState[config.statePreferredKey]);
  const nextPreferred = !currentPreferred;
  const pendingMessage = getTriggerPendingMessage(key, nextPreferred);
  setTriggerTogglePending(key, true, pendingMessage);
  try {
    const response = await apiClient.fetch(AUTO_RECORD_ENDPOINT, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ [config.responseField]: nextPreferred }),
    });
    if (!response.ok) {
      const fallback = `${config.label} request failed (status ${response.status})`;
    ...
    if (payload && typeof payload === "object") {
      applyAutoRecordResponse(payload, { mergeIntoState: true });
    } else {
      updateAutoRecordButton(state.captureStatus);
    }
    button.removeAttribute("title");
  } catch (error) {
    const message = normalizeErrorMessage(error, `${config.label} update failed`);
    console.error(`Failed to update ${config.label}`, error);
    button.title = message;
  } finally {
    setTriggerTogglePending(key, false);
  }
}
...
const {
  ...
  startServicesRefresh,
  stopServicesRefresh,
  fetchRecycleBin: (...args) => fetchRecycleBinDelegate(...args),
  fetchRecordings,
  getRecorderSection,
  ...
  formatIsoDateTime,
  formatBytes,
  getRecordingsRefreshDeferred: () => recordingsRefreshDeferred,
  setRecordingsRefreshDeferred: (value) => {
    recordingsRefreshDeferred = value;
  },
});
...
function updatePlayerActions(record) {
  ...
  if (dom.playerRename) {
    dom.playerRename.disabled =
      isPartial || isRecycle || isRenameDialogPending();
  }
  if (dom.playerDelete) {
...
function setAutoRecordButtonState(active) {
  ...
  if (!nextActive && autoRecordState.motionEnabled) {
    dom.autoToggle.dataset.motionOnly = "true";
    dom.autoToggle.setAttribute(
      "aria-label",
      "Enable auto capture (motion triggers remain active)",
    );
  } else {
    dom.autoToggle.dataset.motionOnly = "false";
    dom.autoToggle.setAttribute(
      "aria-label",
      nextActive ? "Disable auto capture" : "Enable auto capture",
    );
  }
}
...
function updateAutoRecordButton(rawStatus) {
  const status = rawStatus && typeof rawStatus === "object" ? rawStatus : state.captureStatus;
  if (status && typeof status === "object") {
    applyAutoRecordSnapshot(status);
  } else {
    setAutoRecordButtonState(autoRecordState.enabled);
    updateTriggerToggleStates();
  }
  applyTriggerAvailability(status);
  if (!dom.autoToggle) {
    return;
  }
...
function setAutoRecordPending(pending, message = "") {
  ...
  const reason = message || "Auto capture toggle in progress.";
  if (autoRecordState.pending) {
    if (dom.autoToggle) {
      dom.autoToggle.setAttribute("aria-busy", "true");
      setAutoRecordDisabled(true, reason);
    }
    for (const key of TRIGGER_KEYS) {
      setTriggerTogglePending(key, true, reason);
    }
  } else {
  ...
  if (dom.autoToggle) {
    ...
    dom.autoToggle.addEventListener("click", async () => {
      ...
      try {
        ...
        if (!response.ok) {
          const fallback = `Auto capture request failed (status ${response.status})`;
          const message = await extractApiErrorMessage(response, fallback);
          throw new Error(message);
        }
        let payload = null;
        ...
        errorMessage = normalizeErrorMessage(autoError, "Auto capture toggle failed");
        console.error("Auto record toggle failed", autoError);
      }
      setAutoRecordPending(false);
      if (errorMessage && dom.autoToggle) {
        dom.autoToggle.title = errorMessage;
      }
    });
  }

  const triggerUnavailableReason = "Recorder status unavailable.";
  for (const key of TRIGGER_KEYS) {
    const button = getTriggerButton(key);
    const config = TRIGGER_CONFIG[key];
    if (!button || !config) {
    ...
    setTriggerToggleDisabled(key, "status", true, triggerUnavailableReason);
    button.addEventListener("click", () => {
      void handleTriggerToggleClick(key);
    });
  }
