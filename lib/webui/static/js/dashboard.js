...
function applyHealthPayload(payload) {
  ...
  if (resources && typeof resources === "object") {
    ...
    if (temperature && typeof temperature === "object") {
      healthState.resources.temperature = {
        celsius: toFiniteOrNull(temperature.celsius ?? temperature.celsius_c),
        fahrenheit: toFiniteOrNull(temperature.fahrenheit ?? temperature.fahrenheit_f),
        sensor: typeof temperature.sensor === "string" ? temperature.sensor : null,
      };
    } else {
      healthState.resources.temperature = null;
    }
  } else {
    healthState.resources.cpu = null;
    healthState.resources.memory = null;
...
function updatePlayerMeta(record) {
  ...
  if (isRecycle) {
    ...
    if (triggerFlags.rmsBad) {
      details.push("RMS + BAD");
    }
...
function renderRecords(options = {}) {
  ...
  for (const record of records) {
    ...
    if (triggerFlags.rmsBad) {
      const rmsBadPill = document.createElement("span");
      rmsBadPill.className = "meta-pill rmsbad-pill";
      rmsBadPill.dataset.metaRole = "rmsbad-trigger";
      rmsBadPill.textContent = "RMS + BAD";
      mobileMeta.append(rmsBadPill);
    }
  ...
  if (
    ...
    if (metaContainer) {
      ...
      updateMetaPill(
        metaContainer,
        "rmsbad-trigger",
        triggerFlags.rmsBad ? "RMS + BAD" : "",
        "meta-pill rmsbad-pill",
      );
  ...
  try {
    ...
    const normalizedRecords = items.map((item) => {
      ...
      return {
        ...
        detected_bad: Boolean(item.detected_bad),
