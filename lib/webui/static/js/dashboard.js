...
const stateApi =
  (typeof globalThis !== "undefined" && globalThis.TRICORDER_STATE) || null;
...
function apiPath(path) {
  return apiClient.path(path);
}

const eventStreamFactory = createEventStreamFactory({
  window: typeof window !== "undefined" ? window : null,
  eventSource: typeof EventSource !== "undefined" ? EventSource : null,
  logger: typeof console !== "undefined" ? console : undefined,
});

const EVENT_STREAM_SUPPORTED = eventStreamFactory.isSupported();
...
async function fetchSystemHealth() {
  ...
  try {
    const response = await apiClient.fetch(HEALTH_ENDPOINT, { cache: "no-store" });
    if (!response.ok) {
...
async function fetchWebServerSettings({ silent = false } = {}) {
  ...
  try {
    const response = await apiClient.fetch(WEB_SERVER_ENDPOINT, { cache: "no-store" });
    if (!response.ok) {
      throw new Error(`Request failed with ${response.status}`);
    }
    const payload = await response.json();
...
async function saveWebServerSettings() {
  ...
  try {
    ...
    const response = await apiClient.fetch(WEB_SERVER_ENDPOINT, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
...
async function fetchArchivalSettings({ silent = false } = {}) {
  ...
  try {
    const response = await apiClient.fetch(ARCHIVAL_ENDPOINT, { cache: "no-store" });
    if (!response.ok) {
...
async function saveArchivalSettings() {
  ...
  try {
    const response = await apiClient.fetch(ARCHIVAL_ENDPOINT, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
