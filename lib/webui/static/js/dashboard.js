...
let fetchQueuedForce = false;
let fetchQueuedSilent = true;
...
function queueFetchRequest(options = {}) {
  const { silent = true, force = false } = options;
  fetchQueued = true;
  if (force) {
    fetchQueuedForce = true;
  }
  if (silent === false) {
    fetchQueuedSilent = false;
  }
}

function clearFetchQueueState() {
  fetchQueued = false;
  fetchQueuedForce = false;
  fetchQueuedSilent = true;
}

function markRecordingsRefreshDeferred() {
  recordingsRefreshDeferred = true;
  recordingsRefreshPending = true;
}

function shouldDeferDashboardUpdate({ silent = false, force = false } = {}) {
  if (force) {
    return false;
  }
  if (!silent) {
...
function hideStatusRmsIndicator() {
  if (!dom.statusRmsIndicator || !dom.statusRmsIndicatorValue) {
    return;
  }
  dom.statusRmsIndicator.dataset.visible = "false";
  dom.statusRmsIndicator.dataset.trend = "none";
  dom.statusRmsIndicator.setAttribute("aria-hidden", "true");
  if (dom.statusRmsIndicator.dataset.adaptive) {
    delete dom.statusRmsIndicator.dataset.adaptive;
  }
  dom.statusRmsIndicatorValue.textContent = "";
  dom.statusRmsIndicatorValue.removeAttribute("aria-label");
  if (dom.statusRmsIndicatorTrend) {
    dom.statusRmsIndicatorTrend.textContent = "";
    dom.statusRmsIndicatorTrend.dataset.visible = "false";
    dom.statusRmsIndicatorTrend.setAttribute("aria-hidden", "true");
  }
  if (dom.statusRmsIndicatorTrendText) {
    dom.statusRmsIndicatorTrendText.textContent = "";
  }
  statusRmsIndicatorState.visible = false;
  statusRmsIndicatorState.current = null;
  statusRmsIndicatorState.threshold = null;
  statusRmsIndicatorState.valueText = null;
  statusRmsIndicatorState.adaptive = null;
  statusRmsIndicatorState.trend = "none";
}
...
function updateRmsIndicator(rawStatus) {
  ...
  const hasFiniteRms = Number.isFinite(rmsValue);
  const currentWhole = hasFiniteRms ? Math.trunc(rmsValue) : null;
  const validCurrent = Number.isFinite(currentWhole) ? currentWhole : null;
  const currentDisplay = validCurrent !== null ? String(validCurrent) : "--";

  const thresholdWhole =
    adaptiveThreshold !== null && Number.isFinite(adaptiveThreshold)
      ? Math.trunc(adaptiveThreshold)
      : null;
  const validThreshold =
    thresholdWhole !== null && Number.isFinite(thresholdWhole) ? thresholdWhole : null;
  const valueText = validThreshold !== null ? `${currentDisplay}/${validThreshold}` : currentDisplay;

  let trend = "none";
  if (adaptiveEnabled && validThreshold !== null) {
    if (statusRmsIndicatorState.threshold === null) {
      trend = "steady";
    } else if (validThreshold > statusRmsIndicatorState.threshold) {
      trend = "up";
    } else if (validThreshold < statusRmsIndicatorState.threshold) {
      trend = "down";
    } else {
  ...
  if (
    statusRmsIndicatorState.visible &&
    statusRmsIndicatorState.valueText === valueText &&
    statusRmsIndicatorState.adaptive === adaptiveEnabled &&
    statusRmsIndicatorState.trend === trend
  ) {
    return;
  }
  ...
  if (validCurrent !== null) {
    accessibleParts.push(`Current RMS ${validCurrent}`);
  } else {
    accessibleParts.push("Current RMS unavailable");
  }
  if (validThreshold !== null) {
    accessibleParts.push(`Threshold ${validThreshold}`);
  } else if (adaptiveEnabled) {
    accessibleParts.push("Adaptive RMS enabled");
  }

  dom.statusRmsIndicatorValue.textContent = valueText;
  dom.statusRmsIndicatorValue.setAttribute("aria-label", accessibleParts.join(", "));

  dom.statusRmsIndicator.dataset.visible = "true";
  dom.statusRmsIndicator.setAttribute("aria-hidden", "false");
  dom.statusRmsIndicator.dataset.trend = trend;
  if (adaptiveEnabled) {
    dom.statusRmsIndicator.dataset.adaptive = "true";
  } else if (dom.statusRmsIndicator.dataset.adaptive) {
    delete dom.statusRmsIndicator.dataset.adaptive;
  }
  ...
  if (dom.statusRmsIndicatorTrend) {
    ...
    dom.statusRmsIndicatorTrend.textContent = trendSymbol;
    dom.statusRmsIndicatorTrend.dataset.visible = trendVisible ? "true" : "false";
    dom.statusRmsIndicatorTrend.setAttribute("aria-hidden", trendVisible ? "false" : "true");
  }

  if (dom.statusRmsIndicatorTrendText) {
    let trendText = "";
    if (trend === "up") {
      trendText = "Adaptive RMS threshold increased";
    } else if (trend === "down") {
      trendText = "Adaptive RMS threshold decreased";
    } else if (trend === "steady" && adaptiveEnabled && validThreshold !== null) {
      trendText = "Adaptive RMS threshold steady";
    }
    dom.statusRmsIndicatorTrendText.textContent = trendText;
  }

  statusRmsIndicatorState.visible = true;
  statusRmsIndicatorState.current = validCurrent;
  statusRmsIndicatorState.threshold = validThreshold;
  statusRmsIndicatorState.valueText = valueText;
  statusRmsIndicatorState.adaptive = adaptiveEnabled;
  statusRmsIndicatorState.trend = trend;
}
...
function ensurePreviewSectionOrder() {
  ...
  if (
    typeof dom.playerMeta.contains === "function" &&
    dom.playerMeta.contains(dom.clipperContainer)
  ) {
    return;
  }
...
function resetWaveformRmsIndicator() {
...
async function fetchRecordings(options = {}) {
  ...
  recordingsRefreshDeferred = false;
  if (fetchInFlight) {
    queueFetchRequest({ silent, force });
    return;
  }
  fetchInFlight = true;

  if (isRecycleView()) {
    let recycleUpdated = true;
    try {
      recycleUpdated = await fetchRecycleBin({ silent, force });
    } finally {
      fetchInFlight = false;
      if (recycleUpdated) {
        updateStats();
      ...
      if (fetchQueued) {
        const queuedForce = fetchQueuedForce;
        const queuedSilent = fetchQueuedSilent;
        clearFetchQueueState();
        if (recycleUpdated || queuedForce) {
          fetchRecordings({ silent: queuedSilent, force: queuedForce });
        }
      }
    }
    return;
  ...
  const requestedCollection = state.collection;
  const requestedCollectionLabel =
    requestedCollection === "saved" ? "saved" : "recent";
  ...
  params.set("collection", requestedCollectionLabel);

  const endpoint = apiPath(`/api/recordings?${params.toString()}`);
  let deferredDueToInteraction = false;
  try {
    ...
    const staleRequest = state.collection !== requestedCollection;
    if (staleRequest) {
      return;
    }
    ...
    if (total > 0 && normalizedRecords.length === 0 && normalizedOffset < offsetBasis) {
      queueFetchRequest({ silent: true, force: true });
    }
    populateFilters();
    ...
    if (fetchQueued) {
      ...
      const queuedSilent = fetchQueuedSilent;
      clearFetchQueueState();
      if (!deferredDueToInteraction || queuedForce) {
        fetchRecordings({ silent: queuedSilent, force: queuedForce });
      }
    }
  }
