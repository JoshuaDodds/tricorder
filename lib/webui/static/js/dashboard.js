...
let previewRefreshHoldActive = false;
let previewRefreshPending = false;
const hoveredInteractiveElements = new Set();

function isPreviewRefreshHoldActive() {
  return previewRefreshHoldActive;
}

function activatePreviewRefreshHold() {
  if (previewRefreshHoldActive) {
    return;
  }
  previewRefreshHoldActive = true;
  previewRefreshPending = true;
  suspendAutoRefresh();
}

function releasePreviewRefreshHold(options = {}) {
  const { refresh = true } = options;
  const wasActive = previewRefreshHoldActive;
  if (wasActive) {
    previewRefreshHoldActive = false;
  }
  if (wasActive) {
    resumeAutoRefresh();
  }
  if (refresh && (wasActive || previewRefreshPending)) {
...
function resumeAutoRefresh() {
  if (state.recycleBin.open || hoveredInteractiveElements.size > 0 || previewRefreshHoldActive) {
    return;
  }
  if (!autoRefreshSuspended) {
    resumeConfigRefresh();
...
function updateInProgressRecordRow(record) {
  ...
  if (needsDurationPill || needsSizePill || row.querySelector(".record-mobile-meta")) {
    const metaContainer = needsDurationPill || needsSizePill
...
async function fetchRecordings(options = {}) {
  ...
  if (isPreviewRefreshHoldActive() && !force) {
    previewRefreshPending = true;
    recordingsRefreshDeferred = true;
    return;
  }
