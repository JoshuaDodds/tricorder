...
function startServicesRefresh() {
  if (servicesState.timerId || !dom.servicesList) {
    return;
  }
...
function findServiceActionButton(event) {
  if (!event) {
    return null;
  }
  const selector = "button[data-service-unit][data-action]";

  const origin =
    event.target instanceof Element
      ? event.target
      : event.target instanceof Node && event.target.parentElement
      ? event.target.parentElement
      : null;
  if (origin) {
    const directMatch = origin.closest(selector);
    if (directMatch) {
      return directMatch;
    }
  }
  ...
  if (typeof event.composedPath === "function") {
    for (const node of event.composedPath()) {
      ...
      const composedMatch = node.closest(selector);
      if (composedMatch) {
  ...
  while (target && target !== dom.servicesList) {
...
function handleServiceListClick(event) {
  const button = findServiceActionButton(event);
...
function attachEventListeners() {
  ...
  if (dom.servicesList) {
    dom.servicesList.addEventListener("click", handleServiceListClick);
  }
