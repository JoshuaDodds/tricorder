const STATE_MODULE = (() => {
  if (typeof window !== "undefined" && window.TRICORDER_STATE) {
    return window.TRICORDER_STATE;
  }
  if (typeof globalThis !== "undefined" && globalThis.TRICORDER_STATE) {
    return globalThis.TRICORDER_STATE;
  }
  throw new Error("Tricorder state module is unavailable");
})();
...
let eventSourceInitOptionsSupport = EVENT_STREAM_SUPPORTED ? null : false;
let eventSourceInitOptionsWarned = false;
...
const dom = {
  ...
  temperatureDetail: document.getElementById("temperature-detail"),
  ...
  autoToggle: document.getElementById("auto-record-toggle"),
  ...
  waveformMotionSegments: document.getElementById("waveform-motion-segments"),
...
function renderResourceStats() {
  ...
  if (dom.temperatureDetail) {
    let detail = "--";
    if (temperature && typeof temperature === "object") {
      const fahrenheit = toFiniteOrNull(temperature.fahrenheit);
      if (fahrenheit !== null && Number.isFinite(fahrenheit)) {
        const clampedF = clamp(fahrenheit, -100, 392);
        detail = `${clampedF.toFixed(Math.abs(clampedF) >= 100 ? 0 : 1)}°F`;
      } else if (typeof temperature.sensor === "string" && temperature.sensor) {
      ...
      nextSdCard.last_event = {
        timestamp:
          typeof sdCard.last_event.timestamp === "string"
            ...
            : null,
        message:
          typeof sdCard.last_event.message === "string" ? sdCard.last_event.message : "",
      };
    }
  }
  ...
  if (resources && typeof resources === "object") {
    ...
    if (cpu && typeof cpu === "object") {
      nextCpu = {
        ...
        load1m: toFiniteOrNull(typeof cpu.load_1m !== "undefined" ? cpu.load_1m : cpu.load1m),
    ...
    if (temperature && typeof temperature === "object") {
      nextTemperature = {
        celsius: toFiniteOrNull(
          typeof temperature.celsius !== "undefined"
            ? temperature.celsius
            : temperature.celsius_c,
        ),
        fahrenheit: toFiniteOrNull(
          typeof temperature.fahrenheit !== "undefined"
            ? temperature.fahrenheit
            : temperature.fahrenheit_f,
        ),
...
function handlePointerIdleTimeout() {
  ...
  const idleDuration = nowMilliseconds() - lastPointerActivityMs;
  if (idleDuration + 5 < POINTER_IDLE_CLEAR_DELAY_MS) {
    const remaining = Math.max(POINTER_IDLE_CLEAR_DELAY_MS - idleDuration, 0);
...
function recordPointerActivity() {
  lastPointerActivityMs = nowMilliseconds();
  if (hoveredInteractiveElements.size === 0) {
    stopPointerIdleTimer();
    return;
  }
...
function isEventSourceInitOptionsError(error) {
  ...
  return message.includes("constructor") || message.includes("init") || message.includes("credentials");
}
...
function createEventStreamSource(url) {
  ...
  const shouldTryInitOptions = eventSourceInitOptionsSupport !== false;
  if (shouldTryInitOptions) {
    try {
      const source = new EventSource(url, { withCredentials: true });
      eventSourceInitOptionsSupport = true;
      return source;
    } catch (error) {
      if (isEventSourceInitOptionsError(error)) {
        eventSourceInitOptionsSupport = false;
        if (!eventSourceInitOptionsWarned) {
          const detail =
            typeof error === "string"
              ? error
          ...
          if (EVENT_STREAM_REQUIRES_CREDENTIALS) {
            console.warn(
              `Dashboard event stream init options unsupported; falling back to polling updates.${suffix}`,
            );
          } else {
            console.warn(
              `Dashboard event stream init options unsupported; retrying without credentials.${suffix}`,
            );
          }
          eventSourceInitOptionsWarned = true;
        }
        if (EVENT_STREAM_REQUIRES_CREDENTIALS) {
...
function computeRecordsFingerprint(records, options = null) {
  if (!Array.isArray(records) || records.length === 0) {
    return "";
  }
...
function resolveTriggerFlags(source) {
...
function ensureTriggerBadge(container, role, label, className, present) {
  if (!container) {
    return;
  }
...
function renderRecords(options = {}) {
  ...
  for (const record of records) {
    ...
    checkbox.addEventListener("change", (event) => {
      state.selectionFocus = record.path;
      const pendingRange = getPendingSelectionRange();
      if (pendingRange && pendingRange.targetPath === record.path) {
        const { anchorPath, targetPath, shouldSelect } = pendingRange;
        clearPendingSelectionRange();
    ...
    if (record.extension) {
      ...
      typeSpan.dataset.subtextRole = "extension";
...
function updateMetaPill(container, role, text, className = "meta-pill") {
  if (!container) {
    return;
  }
...
function updateSubtextSpan(container, role, text) {
  if (!container) {
    return;
  }
  const normalized = typeof text === "string" ? text.trim() : "";
...
async function loadWaveform(record) {
  ...
  try {
    ...
    const segments = normalizeMotionSegments(payload.motion_segments);
    record.motion_segments = segments;
    waveformState.motionSegments = segments;
...
function attachEventListeners() {
  ...
  if (dom.autoToggle) {
    ...
    dom.autoToggle.addEventListener("click", async () => {
      ...
      try {
        const response = await fetch(AUTO_RECORD_ENDPOINT, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
