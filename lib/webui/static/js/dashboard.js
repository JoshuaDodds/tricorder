...
function formatServiceAction(action) {
  if (!action) {
    return "";
  }
  if (action.confirm) {
...
function formatServiceEntry(entry) {
  if (!entry) {
    return null;
  }
  return {
    ...
    status_state: entry.status_state,
    available: entry.available,
    is_active: entry.is_active,
    auto_restart: entry.auto_restart,
    can_start: entry.can_start,
    can_stop: entry.can_stop,
    related_units: entry.related_units,
...
function renderServiceList(items) {
  ...
  dom.servicesTableBody.textContent = "";
  for (const item of items) {
    const formatted = formatServiceEntry(item);
    if (!formatted) {
      continue;
    }
    const row = document.createElement("tr");
    row.dataset.unit = formatted.unit;
    const nameCell = document.createElement("td");
    nameCell.className = "unit";
    nameCell.textContent = formatted.label;
    row.appendChild(nameCell);
    const statusCell = document.createElement("td");
    statusCell.className = "status";
    statusCell.textContent = formatted.status_text;
    statusCell.dataset.state = formatted.status_state;
    ...
    if (formatted.actions.length === 0) {
      ...
      for (const action of formatted.actions) {
        ...
        button.textContent = formatServiceAction(action);
        if (action.confirm) {
          button.dataset.requiresConfirm = "true";
        }
        actionsCell.appendChild(button);
      }
    }
    row.appendChild(actionsCell);
    dom.servicesTableBody.appendChild(row);
  }
}
...
async function requestServiceAction(unit, action) {
  const endpoint = serviceActionEndpoint(unit);
...
async function handleServiceAction(unit, actionName) {
  try {
    const result = await requestServiceAction(unit, actionName);
    if (result && typeof result === "object" && Array.isArray(result.services)) {
      const normalized = result.services
        .map((entry) => normalizeServiceEntry(entry))
        .filter(Boolean);
      servicesState.items = normalized;
      renderServiceList(normalized);
    }
  } catch (error) {
    console.error(`Failed to perform service action ${actionName} for ${unit}`, error);
  }
}
...
async function fetchServices({ silent = false } = {}) {
  ...
  if (!silent) {
    setServicesStatus("Loading services…", "info");
  }
  try {
    ...
    const normalized = entries.map((entry) => normalizeServiceEntry(entry)).filter(Boolean);
    servicesState.items = normalized;
    renderServiceList(normalized);
    servicesState.lastUpdated = Date.now();
    if (!silent) {
      setServicesStatus("", "");
    }
  } catch (error) {
    ...
    if (!silent) {
      setServicesStatus("Unable to load services.", "error");
    }
  } finally {
...
function appMenuFocusableElements() {
  ...
  return Array.from(nodes).filter((node) => node instanceof HTMLElement && !node.hasAttribute("disabled"));
}
...
function renderServices() {
  ...
  for (const service of servicesState.items) {
    ...
    row.dataset.pending = servicesState.pending.has(service.unit) ? "true" : "false";
    ...
    const createActionButton = (type, labelText) => {
      const button = document.createElement("button");
      button.type = "button";
    ...
    const startButton = createActionButton("start", "Start");
    startButton.disabled = !service.can_start || servicesState.pending.has(service.unit);
    actions.append(startButton);

    const stopButton = createActionButton("stop", "Stop");
    stopButton.disabled = !service.can_stop || servicesState.pending.has(service.unit);
    actions.append(stopButton);

    const reloadButton = createActionButton("reload", "Reload");
    reloadButton.disabled = !service.can_reload || servicesState.pending.has(service.unit);
