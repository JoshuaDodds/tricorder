...
function updateSplitEventButton(rawStatus) {
  ...
  const serviceRunning = isServiceRunning(status);
  if (!serviceRunning) {
...
function isServiceRunning(status) {
  ...
  if (!Object.prototype.hasOwnProperty.call(status, "service_running")) {
    return true;
  }
  return parseBoolean(status.service_running);
}
...
function updateRmsIndicator(rawStatus) {
  ...
  const status = rawStatus && typeof rawStatus === "object" ? rawStatus : null;
  const running = isServiceRunning(status);
...
async function fetchRecorderSection(key) {
  ...
  try {
    ...
    const payload = await response.json();
    if (key === "audio") {
      if (payload && Array.isArray(payload.available_devices)) {
        recorderDialogState.availableAudioDevices = payload.available_devices;
      } else {
        recorderDialogState.availableAudioDevices = [];
      }
    }
    const canonical = typeof section.options.fromResponse === "function"
...
function updateAutoRecordButton(rawStatus) {
  ...
  } else if (!isServiceRunning(status)) {
    disabled = true;
    const stopReason =
