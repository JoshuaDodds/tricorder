...
const dom = {
  ...
  manualRecord: document.getElementById("manual-record-toggle"),
...
if (dom.manualRecord) {
  dom.manualRecord.dataset.defaultLabel = dom.manualRecord.textContent || "Manual Record";
  dom.manualRecord.dataset.activeLabel = "Stop Manual";
  dom.manualRecord.dataset.pendingLabel = "Updating…";
  dom.manualRecord.setAttribute("aria-pressed", "false");
...
function setManualRecordDisabled(disabled, reason = "") {
  manualRecordState.disabled = Boolean(disabled);
  manualRecordState.disabledReason = manualRecordState.disabled ? reason : "";
  if (!dom.manualRecord) {
    return;
  }
  const nextDisabled = manualRecordState.disabled || manualRecordState.pending;
  if (dom.manualRecord.disabled !== nextDisabled) {
    dom.manualRecord.disabled = nextDisabled;
  }
  if (nextDisabled) {
    const message = manualRecordState.disabledReason;
    if (message) {
      dom.manualRecord.title = message;
    } else if (!manualRecordState.pending) {
      dom.manualRecord.removeAttribute("title");
    }
    dom.manualRecord.setAttribute("aria-disabled", "true");
  } else {
    dom.manualRecord.removeAttribute("title");
    dom.manualRecord.removeAttribute("aria-disabled");
  }
}

function setManualRecordPending(pending) {
  const nextPending = Boolean(pending);
  if (manualRecordState.pending === nextPending) {
    return;
  }
  manualRecordState.pending = nextPending;
  if (!dom.manualRecord) {
    return;
  }
  if (nextPending) {
    dom.manualRecord.setAttribute("aria-busy", "true");
    const label = dom.manualRecord.dataset.pendingLabel || "Updating…";
    dom.manualRecord.textContent = label;
  } else {
    dom.manualRecord.removeAttribute("aria-busy");
    updateManualRecordButton();
  }
  setManualRecordDisabled(manualRecordState.disabled, manualRecordState.disabledReason);
}

function updateManualRecordButton(source) {
  if (!dom.manualRecord) {
    return;
  }
  let enabled = manualRecordState.enabled;
  const payload = source && typeof source === "object" ? source : undefined;
  if (payload && Object.prototype.hasOwnProperty.call(payload, "manual_recording")) {
    enabled = parseBoolean(payload.manual_recording);
  } else if (payload && Object.prototype.hasOwnProperty.call(payload, "enabled")) {
    enabled = parseBoolean(payload.enabled);
  }
  manualRecordState.enabled = Boolean(enabled);
  dom.manualRecord.setAttribute("aria-pressed", manualRecordState.enabled ? "true" : "false");
  if (!manualRecordState.pending) {
    dom.manualRecord.textContent = manualRecordState.enabled
      ? dom.manualRecord.dataset.activeLabel || "Stop Manual"
      : dom.manualRecord.dataset.defaultLabel || "Manual Record";
  }

  let serviceRunning = true;
  if (payload && Object.prototype.hasOwnProperty.call(payload, "service_running")) {
    serviceRunning = parseBoolean(payload.service_running);
  } else if (!payload && state.captureStatus && typeof state.captureStatus === "object") {
    serviceRunning = parseBoolean(state.captureStatus.service_running);
  }

  if (!serviceRunning) {
    setManualRecordDisabled(true, "Recorder service offline.");
  } else {
    setManualRecordDisabled(false);
  }
}

async function requestManualRecordChange(enabled) {
  if (!dom.manualRecord || manualRecordState.pending) {
    return;
  }
  setManualRecordPending(true);
  try {
    ...
    if (!response.ok) {
      let message = `Manual record toggle failed (status ${response.status})`;
      try {
    ...
    const payload = await response.json();
    updateManualRecordButton(payload);
    await fetchRecordings({ silent: true });
  } catch (error) {
    console.error("Manual record toggle failed", error);
    const message = error instanceof Error && error.message ? error.message : "Unable to toggle manual recording.";
...
async function fetchRecordings(options = {}) {
  ...
  try {
    ...
    setManualRecordDisabled(true, "Recorder offline.");
    setManualRecordPending(false);
    updateManualRecordButton();
    fetchQueued = false;
  } finally {
...
function updateAudioFilterControls() {
  ...
  for (const stage of stages) {
    ...
    const enabled = toggle instanceof HTMLInputElement ? toggle.checked : true;
...
function renderRecycleBinItems() {
  ...
  for (const item of state.recycleBin.items) {
    ...
    const nameCell = document.createElement("td");
    nameCell.textContent = item.name && item.name.trim() ? item.name : item.original_path || item.id;
    row.append(nameCell);

    const pathCell = document.createElement("td");
    pathCell.textContent = item.original_path || "";
    if (item.restorable === false) {
      const conflict = document.createElement("span");
      conflict.className = "recycle-bin-conflict";
      conflict.textContent = "In use";
      pathCell.append(conflict);
    }
    row.append(pathCell);
...
async function fetchRecycleBin(options = {}) {
  ...
  try {
    ...
    if (!silent) {
      window.alert("Unable to load recycle bin entries.");
    }
  } finally {
...
function attachEventListeners() {
  ...
  if (dom.manualRecord) {
    dom.manualRecord.addEventListener("click", () => {
      if (manualRecordState.pending) {
        return;
      }
      const next = !manualRecordState.enabled;
      requestManualRecordChange(next);
    });
  }
