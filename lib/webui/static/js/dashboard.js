...
function computeRecordsFingerprint(records, options = null) {
  ...
  for (const record of records) {
    ...
    if (skipPartialVolatile) {
      parts.push(
        `${path}|${isPartial ? "partial" : "final"}|${safeModified}|${safeSize}|${safeDuration}|${safeTrigger}|${safeRelease}|${safeMotionTrigger}|${safeMotionRelease}|${safeMotionStarted}|${safeMotionReleased}|${safeWaveform}`
      );
    } else {
      parts.push(
        `${path}|${safeModified}|${safeSize}|${safeDuration}|${safeTrigger}|${safeRelease}|${safeMotionTrigger}|${safeMotionRelease}|${safeMotionStarted}|${safeMotionReleased}|${safeWaveform}`
      );
    }
  }
  return parts.join("\n");
...
function updateMetaPill(container, role, text, className = "meta-pill") {
  ...
  const existing = container.querySelector(`[data-meta-role="${role}"]`);
  if (normalized) {
    if (existing) {
...
function updateSubtextSpan(container, role, text) {
  ...
  const normalized = typeof text === "string" ? text.trim() : "";
  const existing = container.querySelector(`[data-subtext-role="${role}"]`);
  if (normalized) {
    if (existing) {
      existing.textContent = normalized;
    } else {
