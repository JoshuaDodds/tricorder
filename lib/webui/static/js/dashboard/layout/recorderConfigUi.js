export function createRecorderConfigUi(deps) {
  ...
  const CUSTOM_AUDIO_DEVICE_VALUE = "__custom__";
  ...
  function registerRecorderSections() {
    ...
    const audioSection = recorderDom.sections.audio;
    if (audioSection && audioSection.deviceSelect) {
      audioSection.deviceSelect.addEventListener("change", () => {
        const selection = audioSection.deviceSelect.value;
        if (selection === CUSTOM_AUDIO_DEVICE_VALUE) {
          toggleAudioDeviceCustom(true);
          if (audioSection.device) {
            audioSection.device.focus();
          }
        } else {
          if (audioSection.device) {
            audioSection.device.value = selection;
          }
          toggleAudioDeviceCustom(false);
        }
      });
    }
  ...
  function resolveAvailableAudioDevices() {
    const devices = recorderDialogState.availableAudioDevices;
  ...
  function ensureAudioDeviceOptions(select, devices) {
    if (!select) {
      return;
    }
    ...
    defaultOption.value = "";
    defaultOption.textContent = "Use ALSA default";
    ...
    customOption.value = CUSTOM_AUDIO_DEVICE_VALUE;
    customOption.textContent = "Custom device…";
    select.append(customOption);
    select.dataset.optionsSignature = signature;
  }

  function toggleAudioDeviceCustom(show) {
    const section = recorderDom.sections.audio;
    if (!section || !section.deviceCustomWrapper) {
      return;
    }
    if (show) {
      section.deviceCustomWrapper.hidden = false;
      section.deviceCustomWrapper.setAttribute("aria-hidden", "false");
    } else {
      section.deviceCustomWrapper.hidden = true;
      section.deviceCustomWrapper.setAttribute("aria-hidden", "true");
    }
  }
  ...
  function applyAudioForm(data) {
    ...
    const devices = resolveAvailableAudioDevices();
    const deviceValue =
      data && typeof data.device === "string" ? data.device.trim() : "";
    if (section.deviceSelect) {
      ensureAudioDeviceOptions(section.deviceSelect, devices);
      let selection = "";
      if (deviceValue && devices.some((device) => device.id === deviceValue)) {
        selection = deviceValue;
      } else if (deviceValue) {
        selection = CUSTOM_AUDIO_DEVICE_VALUE;
      } else if (devices.length === 0) {
        selection = CUSTOM_AUDIO_DEVICE_VALUE;
      }
      section.deviceSelect.value = selection;
      toggleAudioDeviceCustom(selection === CUSTOM_AUDIO_DEVICE_VALUE);
    } else {
      toggleAudioDeviceCustom(Boolean(deviceValue));
    }
    if (section.device) {
      section.device.value = deviceValue;
    }
    if (section.sampleRate) {
      section.sampleRate.value = String(data.sample_rate);
    }
    if (section.channels) {
      const channelValue = Number(data.channels);
      section.channels.value = channelValue === 2 ? "2" : "1";
    }
    if (section.frameMs) {
    ...
    if (section.useUsbResetWorkaround) {
      section.useUsbResetWorkaround.checked = Boolean(data.use_usb_reset_workaround);
    }
    const filters =
  ...
  function readAudioForm() {
    ...
    let deviceValue = "";
    if (section.deviceSelect) {
      const selection = section.deviceSelect.value;
      if (selection === CUSTOM_AUDIO_DEVICE_VALUE) {
        deviceValue = section.device ? section.device.value : "";
      } else {
        deviceValue = selection;
      }
    } else if (section.device) {
      deviceValue = section.device.value;
    }
    ...
    const payload = {
      ...
      use_usb_reset_workaround: section.useUsbResetWorkaround
        ? section.useUsbResetWorkaround.checked
        : undefined,
      filter_chain: {
        denoise: {
