...
export function updateWaveformMarkers() {
  ...
  const currentRecord =
    stateRef && typeof stateRef === "object" && stateRef.current && typeof stateRef.current === "object"
      ? stateRef.current
      : null;
  const isActiveRecording = Boolean(currentRecord && currentRecord.inProgress === true);
  ...
  let releaseSeconds = currentRecord
    ? toFiniteOrNullRef(currentRecord.release_offset_seconds)
    : toFiniteOrNullRef(stateRef.current?.release_offset_seconds);
  if (!Number.isFinite(releaseSeconds)) {
    if (!isActiveRecording && Number.isFinite(configStateRef?.postPadSeconds)) {
  ...
  if (releaseSeconds !== null && triggerSeconds !== null && releaseSeconds < triggerSeconds) {
