export function createRecycleBinHelpers({
  ...
  function recycleBinAudioUrl(id, { download = false, raw = false } = {}) {
    if (typeof id !== "string" || !id) {
      return "";
    }
  ...
  function recycleBinRecordFromItem(item) {
    ...
    const rawAvailable = item.raw_audio_available === true;
    const rawName =
      typeof item.raw_audio_name === "string" && item.raw_audio_name ? item.raw_audio_name : "";
    const rawPath =
      typeof item.raw_audio_path === "string" && item.raw_audio_path ? item.raw_audio_path : "";
    return {
      path: `recycle-bin/${id}`,
      name,
      extension,
      ...
      recycleBinRawAvailable: rawAvailable,
      recycleBinRawName: rawName,
    };
  }
