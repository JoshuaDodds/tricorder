(() => {
  ...
  if (isPartial) {
    ...
    const buildDownloadLinks = () => {
      const links = [];
      const hasRawDownload =
        typeof recordHasRawAudio === "function" ? Boolean(recordHasRawAudio(record)) : false;

      const processedLink = document.createElement("a");
      processedLink.textContent = "Download";
      processedLink.classList.add("ghost-button", "small");
      const processedUrl =
        typeof recordAudioUrl === "function"
          ? recordAudioUrl(record, { download: true })
          : resolvePlaybackSourceUrl(record, { download: true });
      if (processedUrl) {
        processedLink.href = processedUrl;
        processedLink.removeAttribute("aria-disabled");
      } else {
        processedLink.removeAttribute("href");
        processedLink.setAttribute("aria-disabled", "true");
      }
      const processedName =
        typeof getProcessedDownloadName === "function"
          ? getProcessedDownloadName(record)
          : typeof resolveRecordDownloadName === "function"
            ? resolveRecordDownloadName(record)
            : "";
      if (processedName) {
        processedLink.download = processedName;
        processedLink.setAttribute("download", processedName);
      } else {
        processedLink.download = "";
        processedLink.removeAttribute("download");
      }
      processedLink.addEventListener("click", (event) => {
      ...
      if (hasRawDownload) {
        ...
        rawLink.textContent = "Download stereo";
        rawLink.classList.add("ghost-button", "small");
        const rawUrl =
          typeof recordRawAudioUrl === "function"
            ? recordRawAudioUrl(record, { download: true })
            : "";
        if (rawUrl) {
          rawLink.href = rawUrl;
        ...
        const rawName =
          typeof getRawDownloadName === "function" ? getRawDownloadName(record) : "";
        if (rawName) {
          rawLink.download = rawName;
          rawLink.setAttribute("download", rawName);
        } else {
          rawLink.download = "";
    ...
    if (!isRecycleRecord) {
      ...
      actionsRow.append(renameButton, deleteButton);
    } else {
      const downloadLinks = buildDownloadLinks();
      for (const link of downloadLinks) {
        actionsRow.append(link);
