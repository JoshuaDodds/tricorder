...
audio:
  ...
  # Software gain multiplier (linear). Default 1.0 (unity) leaves samples untouched.
  # Increase only when the capture hardware is too quiet; prefer physical gain knobs first.
  # Values above ~2.0 (~+6 dB) can clip and inflate RMS quickly on loud sources.
  gain: 1.0
...
adaptive_rms:
  ...
  # Lowest RMS value the adaptive trigger can fall back to. Leave null to reuse
  # segmenter.rms_threshold so the adaptive gate never dips below your manual setting.
  # Example: 275 keeps the trigger above a quiet-room floor that still needs a buffer.
  min_rms: null

  # Advanced: minimum normalized RMS threshold (0.0–1.0). Most installs can ignore this
  # and rely on min_rms + segmenter.rms_threshold.
  min_thresh: 0.01
